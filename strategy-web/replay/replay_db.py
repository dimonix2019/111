...
def _row_timestamp_ms(row: dict[str, Any]) -> int:
    """Рабочий ms бара: 0 / пусто из sqlite (дата без времени) не должно
    всплывать первым при склейке 1095d+255d и резать 1м-окно до 2026."""
    try:
        ms = int(row.get("timestampMs") or 0)
    ...
    return int(parse_ts_ms(str(row.get("tradeDate") or "")))
...
def merge_bars_by_timestamp(
    ...
    """Склеить историю и живой хвост; одинаковый timestampMs берётся из tail."""
    merged: dict[Any, dict[str, Any]] = {}
    for src in (early or [], tail or []):
        for row in src:
            ms = _row_timestamp_ms(row)
            if ms <= 0:
                continue
            if int(row.get("timestampMs") or 0) != ms:
                row = {**row, "timestampMs": ms}
            merged[ms] = row
...
def _load_cached_bars(
    ...
    # Не подменять CSV целиком sqlite: бар с timestamp_ms=0 всплывал первым
    # и склейка 1095d+255d теряла 2023–2025 для tip1m.
    if csv_path.is_file() and not _db_covers_csv_history(csv_path):
        ...
        if bars:
            ...
            try:
                ...
                db_bars = []
            if db_bars:
                bars = merge_bars_by_timestamp(bars, db_bars)
                if not large:
