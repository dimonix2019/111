  ...
  return (
    <div className="space-y-2">
      ...
      {bars.length === 0 ? (
        ...
        <HistogramBars bars={bars} chartHeight={isTime && timeGranularityMin < 60 ? 88 : 96} scrollable={isTime && timeGranularityMin < 60} />
      )}
    </div>
  )
}

export function TradeDistributionPanel({ trades, totalCount, pack }: Props) {
  ...
  return (
    <Panel title="Распределения">
      ...
      <motion div className="mb-3 flex flex-wrap gap-1.5">
        {QUICK_IDS.map((id) => {
