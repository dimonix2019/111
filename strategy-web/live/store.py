...
def _init(conn: sqlite3.Connection) -> None:
    ...
    # One-shot: old conservative Short enter 6.2 → Prod/Test 6.1 (addon exit follows base).
    mig61 = conn.execute(
        "SELECT 1 FROM live_settings WHERE key = 'spread_enter_wide_v61'"
    ).fetchone()
    if mig61 is None:
        def _as_f(raw: str | None) -> float | None:
        ...
        ew_row = conn.execute(
            "SELECT value FROM live_settings WHERE key = 'spread_enter_wide'"
        ).fetchone()
        ew_val = _as_f(ew_row["value"] if ew_row else None)
        if ew_val is None or abs(ew_val - 6.2) < 1e-9:
            conn.execute(
                "INSERT INTO live_settings(key, value) VALUES('spread_enter_wide', ?) "
                "ON CONFLICT(key) DO UPDATE SET value = excluded.value",
                (str(DEFAULT_SPREAD_ENTER_WIDE),),
            )
        ax_row = conn.execute(
            "SELECT value FROM live_settings WHERE key = 'addon_exit_wide'"
        ).fetchone()
        ax_val = _as_f(ax_row["value"] if ax_row else None)
        if ax_val is not None and abs(ax_val - 6.2) < 1e-9:
            conn.execute(
                "INSERT INTO live_settings(key, value) VALUES('addon_exit_wide', ?) "
                "ON CONFLICT(key) DO UPDATE SET value = excluded.value",
                (str(DEFAULT_SPREAD_ENTER_WIDE),),
            )
        conn.execute(
            "INSERT INTO live_settings(key, value) VALUES('spread_enter_wide_v61', '1')"
        )
...
def get_settings_bundle() -> dict[str, Any]:
    ...
    return {
        ...
        "spread_enter_wide": float(
            get_setting("spread_enter_wide", str(DEFAULT_SPREAD_ENTER_WIDE))
            or str(DEFAULT_SPREAD_ENTER_WIDE)
        ),
        "spread_exit_wide": float(
            get_setting("spread_exit_wide", str(DEFAULT_SPREAD_EXIT_WIDE))
            or str(DEFAULT_SPREAD_EXIT_WIDE)
        ),
        "spread_enter_narrow": float(
            get_setting("spread_enter_narrow", str(DEFAULT_SPREAD_ENTER_NARROW))
            or str(DEFAULT_SPREAD_ENTER_NARROW)
        ),
        "spread_exit_narrow": float(
            get_setting("spread_exit_narrow", str(DEFAULT_SPREAD_EXIT_NARROW))
            or str(DEFAULT_SPREAD_EXIT_NARROW)
        ),
