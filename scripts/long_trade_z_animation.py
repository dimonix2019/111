#!/usr/bin/env python3
"""Анимация LONG round-trip: свечной Z-график, пересечение −entry/−exit на соседних 15м барах."""
...
ENTRY = 0.7
EXIT = 0.5
OUT = Path("/opt/cursor/artifacts/long_trade_z_animation.mp4")
CANDLE_W = 0.55
BULL = "#16a34a"
BEAR = "#dc2626"
WICK = "#374151"
...
# LONG: EnterLong 06:45 (prev −0.55 > −0.7, close −0.85 ≤ −0.7); ExitLong 10:00 (prev −0.56 < −0.5, close −0.42 ≥ −0.5)
Z_CLOSE = np.array([
...
def main() -> None:
    ...
    ax.set_title("LONG: свечной Z-график · пересечение порога на соседних барах", fontsize=13, fontweight="bold", pad=12)
    ...
    cross_ann = ax.annotate(
        ...
        fontsize=10, color="#065f46", fontweight="bold", visible=False, zorder=10,
    )
    shade = ax.axvspan(0, 0, ymin=0, ymax=1, alpha=0.10, color=LONG_SHADE, visible=False, zorder=0)

    bull_patch = mpatches.Patch(color=BULL, label="Z↑ (close ≥ open)")
    bear_patch = mpatches.Patch(color=BEAR, label="Z↓ (close < open)")
    long_patch = mpatches.Patch(color=LONG_SHADE, alpha=0.2, label="LONG")
    ax.legend(handles=[bull_patch, bear_patch, long_patch], loc="lower left", fontsize=8, framealpha=0.9)
    ...
    def frame_title(i: int) -> str:
        ...
        if i < EXIT_IDX:
            return f"Бар {LABELS[i]}: LONG открыт, close Z={C[i]:.2f}"
        if i == EXIT_IDX:
            return f"Бар {LABELS[i]}: выход LONG, close Z={C[i]:.2f}"
    ...
    def update(frame: int):
        ...
        if idx < ENTRY_IDX:
            ...
            pos_text.set_text("Позиция: LONG")
            pos_text.set_bbox(dict(boxstyle="round,pad=0.4", facecolor="#d1fae5", edgecolor="#059669", alpha=0.95))
            shade.set_visible(True)
            shade.set_xy((ENTRY_IDX - 0.5, y_lo))
            shade.set_width(max(0.15, idx - ENTRY_IDX + 1.0))
            shade.set_height(y_hi - y_lo)
        else:
            pos_text.set_text("Позиция: FLAT")
    ...
    writer = FFMpegWriter(fps=6, metadata={"title": "LONG Z-candles"}, bitrate=2800)
