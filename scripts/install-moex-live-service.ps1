...
# NSSM install resets the DACL; re-grant start/stop for this admin user (no extra UAC).
$grantAcl = Join-Path $PSScriptRoot "grant-restart-acl.ps1"
if (Test-Path $grantAcl) {
    Write-Host "Granting current user start/stop on $ServiceName ..."
    & $grantAcl -ServiceName $ServiceName -NoRestart
}
