...
2.0.42 — «Сделка»: открытая пара с Т‑Инвест (GetOperations) больше не теряется — строка «открыта» в таблице и карточка позиции, даже если GetPortfolio без ticker. TATN/TATNP по FIGI и «Татнефть-п».
2.0.41 — «Сделка»: автообновление PnL и портфеля раз в 5 с, пока вкладка открыта (история сделок и фоновый опрос — как раньше, раз в 15 с).
