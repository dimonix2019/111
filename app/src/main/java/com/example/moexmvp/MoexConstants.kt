...
2.0.49 — Вкладка «Настройки»: пороги срабатывания, подвкладки «Лог приложения» и «Лог биржи» (скачать/сохранить/файл, тумблер записи). В UI последние 20 строк, полный журнал ведётся. Логи убраны с «О приложении» и «Сделка».
2.0.48 — «Сделка»: в лог пишутся ответы биржи (PostOrder / GetMaxLots: HTTP, code, status, лоты). Карточка «Ответы биржи» на вкладке. 12.09 Short: TATN ушла, TATNP — HTTP 400 code=3 Not enough assets.
2.0.47 — «Сделка»: перед входом GetMaxLots. Если брокер даёт 0 лот в шорт TATN (выходной дилер / нет займа), Short не шлётся — тост с причиной. Long при этом мог открываться, потому что шортится TATNP.
2.0.46 — «Сделка»: герой снова едет вместе с ценой портфеля. Залипший дилерский стакан сдвигается на last GetPortfolio; опрос не пропускается и не виснет на GetOrderBook дольше 8 с.
2.0.45 — «Сделка»: выходной дилерский стакан T‑Invest (ORDERBOOK_TYPE_DEALER) + VWAP. 12.09 16:07 last±0,14% дало −197 ₽ при факте −157 ₽ — биржа пустая, TATN продали почти по last.
"""
