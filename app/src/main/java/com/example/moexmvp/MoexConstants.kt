...
2.0.11 — Trade Desk / шторка: «подготовка к закрытию» в режиме спред-уровней по S (L вых 4 / S вых 5,8), не по Z ±1,3; в чеклисте метрика S%.
2.0.10 — T‑Invest: «Список счетов» / портфель берут токен из поля (сразу сохраняют), а не старый с диска; пустой токен — подсказка «вставьте и сохраните»; подсказка при HTTP 401.
2.0.9 — T‑Invest Prod: доверие CA Минцифры + user-CA (фикс «Trust anchor for certification path not found» на invest-public-api).
"""
