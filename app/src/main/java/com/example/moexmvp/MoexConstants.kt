...
2.0.56 — «Рынок» в выходные: текущий спред и 1м свеча идут из read-only дилерского стакана T‑Invest каждые 15 с, с явной подписью «внебиржа». Фоновый монитор проверяет внебиржевой спред раз в минуту.
2.0.55 — Фоновый монитор: постоянный сигнал оформлен как special-use FGS, в закрытую сессию тяжёлый расчёт не чаще 15 мин, шторка обновляется раз в минуту. Убраны 5-минутные update-alarm/дубли проверок. `OTHER` после установки отделяется от runtime-exit.
2.0.54 — Фоновый монитор: 10-секундный пульс больше не читает 14 тыс. баров и не ходит в сеть; недоступный WebDesk уходит в backoff до 15 мин; повторные старты сервиса подавлены. В лог добавлена причина прошлого завершения процесса Android.
2.0.53 — «Рынок»: ANR / «coroutine scope left the composition» — отмена Compose больше не ловится как ошибка и не крутит ISS. На выходных после первой загрузки 1м не дёргаем. 4,09% в вс — пятничный close.
2.0.52 — «Рынок»: 4,09% два часа подряд — это пятничный close, не живая минута. Больше не подставляем вчерашний 10м в «сейчас». Бар — время последней сделки (11.09 23:49), плюс «биржа закрыта».
"""
